...
- `infra/deploy/Dockerfile` (API+UI combinados, `requirements/base.txt+api.txt`, sem `requirements/train.txt`), carrega `data/model.pkl` **committed no repo** (não de um MLflow ao vivo — decisão do Passo 9), front-matter YAML do HF Space, segredos configurados na UI do Space (não versionados).
